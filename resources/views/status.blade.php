...
<body>
    ...
    <header class="w3-container w3-red w3-center" style="padding:80px 16px">
        <h1 class="w3-margin w3-jumbo">Status</h1>
      </header>
<h2 style="text-align:center">Items List</h2>
      ...
      <table>
        ...
        @foreach($selldatas ?? '' as $selldata)
        <tr>
          <td>{{ $selldata->productname }}</td>
          <td>{{ $selldata->quantity }}</td>
          <td>{{ $selldata->desc }}</td>
          <td>{{ $selldata->status }}</td>
          <td><img src="data:image/png;base64,{{ chunk_split(base64_encode($selldata->image)) }}" height="100" width="100">            <br></td>
        </tr>
        @endforeach
      </table>
      <footer>
